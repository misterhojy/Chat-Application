...
class Server:
    ...
    def handle_disconnect(self, msg, client_addr):
        del self.users[msg[2]]
        # del self.dat[client_addr]
    ...
    def send_ack(self, seqno, client_addr):
        print("sending ACK")
        packet = util.make_packet("ack", seqno+1, "") # send/end packet
        print(packet)
        self.sock.sendto(packet.encode('utf-8'), client_addr) # send packet to server
    ...
    def start(self):
        ...
        while True:
            ...
            if not msg:
                print("NONE")
                # upon receving a start, create a new key in a dict
                # if (msg_type == "start"):
                #     self.can_send = False
                #     self.dat[client_addr] = ""
                # # upon receving an end, delete the key
                # elif (msg_type == "end"):
                #     self.can_send = True
                self.send_ack(int(seqno), client_addr)

            else:
                # combine msg to its correct place in the dict
                # self.dat[client_addr].append(' '.join(msg[2:]))
                match msg[0]:
                    case "join":
                    ...
                    case "request_users_list":
                        ...
                        self.send_ack(int(seqno), client_addr)
                    case "disconnect":
                        self.handle_disconnect(msg, client_addr)
                        self.send_ack(int(seqno), client_addr)
                    case "send_message":
                        print("SOMEONE IS SENDING")
                        self.send_ack(int(seqno), client_addr)
